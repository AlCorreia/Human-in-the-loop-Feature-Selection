        ...
        if meta is not None:
            ...
            if second_conv:
                ...
                zoom_X = tf.layers.conv2d(
                            inputs= tf.reshape(zoom_X, [-1, 30, 30, self.num_filters]),
...
class Raw_Bernoulli_Net(Model):
    ...
    def __init__(self, layer_sizes, optimizer, num_filters, num_features, num_samples, frame_size, learning_rate, feedback_distance, directory, second_conv=False, reg=1.0, meta=None):
        ...
        if meta is not None:
            ...
            self.saver.restore(self.sess, tf.train.latest_checkpoint("CRG_cosine_nopretrain"))
            graph = tf.get_default_graph()
            self.X = graph.get_tensor_by_name("X:0")
            self.label = graph.get_tensor_by_name("y:0")
            self.feedback = graph.get_tensor_by_name("feedback:0")
            probs_logits = graph.get_tensor_by_name("attention/pre_activation:0")
            ...
            # Multiply the features by the gates defined by attention
            zoom_X = tf.multiply(self.X, tf.reshape(self.attention, [-1, self.num_features]), name='Zoom_X')

            conv = tf.layers.conv2d(
                inputs= tf.reshape(zoom_X, [-1, 60, 60, 1]),
                filters=self.num_filters,
                kernel_size=[4, 4],
                padding="same",
                strides=(2, 2),
            ...
            input_images = tf.summary.image("Visualize_Input", tf.reshape(self.X, [10, frame_size, frame_size, 1]), max_outputs=10)
            prob_images = tf.summary.image("Probability_Distribution", tf.reshape(self.probs, [10, 60, 60, 1]), max_outputs=10)
            feedback_images = tf.summary.image("Feedback_Images", tf.reshape(self.feedback, [10, 60, 60, 1]), max_outputs=10)
            attention_images = tf.summary.image("Attention_Images", tf.reshape(self.attention, [10, 60, 60, 1]), max_outputs=10)
...
class Raw_Gumbel_Net(Model):
    ...
    def __init__(self, layer_sizes, optimizer, num_filters, num_features, frame_size, num_cat, learning_rate, feedback_distance, directory, second_conv=False, initial_tau=10.0, reg=1.0, meta=None):
        ...
        if meta is not None:
            ...
            zoom_X = tf.multiply(self.X, tf.reshape(attention, [-1, self.num_features]), name='Zoom_X')
            ...
            input_images = tf.summary.image("Visualize_Input", tf.reshape(self.X, [10, frame_size, frame_size, 1]), max_outputs=10)
            prob_images = tf.summary.image("Probability_Distribution", tf.reshape(attention, [10, 60, 60, 1]), max_outputs=10)
            feedback_images = tf.summary.image("Feedback_Images", tf.reshape(self.feedback, [10, 60, 60, 1]), max_outputs=10)
            attention_images = tf.summary.image("Attention_Images", tf.reshape(self.attention, [10, 60, 60, 1]), max_outputs=10)
